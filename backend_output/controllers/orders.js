...
exports.createOrder = async (req, res) => {
  ...
  // rebel-engine.js sends flat fields; backend expects nested objects.
  // We accept both shapes here so the frontend never needs to change.
  const body = req.body;
  ...
  // items: frontend sends [{ productId, quantity }], backend expects [{ product: { id }, quantity }]
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      for (const item of items) {
        const product = await Product.findOneAndUpdate(
          { _id: item.product.id, clientId: req.clientId, stockQuantity: { $gte: item.quantity }, status: 'active' },
      ...
      // Build EFT details if applicable
      ...
      if (paymentMethod === 'eft' && req.client?.banking) {
        ...
        eftDetails = {
          ...
          reference:      `${b.eftReferencePrefix || 'ORD'}-TBD` // updated after orderNumber assigned
        };
      }
      ...
      // Patch EFT reference now that we have the orderNumber
      if (eftDetails) {
    ...
    return res.status(isStockOut ? 409 : 500).json({
      success: false,
      message: isStockOut ? 'A product ran out of stock during checkout.' : 'Order creation failed.'
    });
  } finally {
...
exports.markAsPaid = async (req, res) => {
  ...
  try {
    ...
    // Update customer stats — non-critical, outside transaction
...
exports.updateFulfillment = async (req, res) => {
  ...
  // FIX: includes 'completed' to match Order.js enum
...
// FIX 8: Added try/catch — was the only async controller without error handling
